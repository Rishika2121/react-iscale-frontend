    ...
    <div style={{ minHeight: '100vh', background: '#f9f9f9', paddingBottom: 80 }}>
      ...
      <div style={{ 
        ...
        <div style={{ position: 'relative', zIndex: 1, textAlign: 'center' }}>
          <h1 style={{ fontSize: 40, fontWeight: 800, color: '#111', marginBottom: 8 }}>Allied College</h1>
          <div style={{ color: '#555', fontSize: 13, fontWeight: 500 }}>
             Home <span style={{ margin: '0 6px' }}>›</span> Allied College
          </div>
        </div>
      </div>
      ...
      <div className="container" style={{ maxWidth: 1200, margin: '0 auto', padding: '60px 24px' }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(240px, 1fr))', gap: 20 }}>
          {colleges.map((college, idx) => (
            <div key={idx} style={{
              background: '#fff', borderRadius: 8, padding: '24px 16px 16px', 
              textAlign: 'center', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center',
              boxShadow: '0 2px 10px rgba(0,0,0,0.04)', border: '1px solid #eaeaea',
              transition: 'all 0.3s ease', cursor: 'pointer', height: 200
            }}
            onMouseEnter={e => { e.currentTarget.style.transform = 'translateY(-4px)'; e.currentTarget.style.boxShadow = '0 12px 24px rgba(0,0,0,0.08)'; e.currentTarget.style.borderColor = '#ddd'; }}
            onMouseLeave={e => { e.currentTarget.style.transform = 'none'; e.currentTarget.style.boxShadow = '0 2px 10px rgba(0,0,0,0.04)'; e.currentTarget.style.borderColor = '#eaeaea'; }}
            >
              {college.img && (
