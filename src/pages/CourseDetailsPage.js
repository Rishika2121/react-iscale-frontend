...
import { Play, Download, Eye, Calendar, Globe, ChevronDown, ChevronUp, FileText, CheckCircle, XCircle, CreditCard, Video, ArrowRight } from 'lucide-react';
...
,
  ...
  'master-of-data-analytics-program': {
    ...
    videoUrl: 'https://www.youtube.com/embed/fAwe3_M9d_c',
    thumbnail: 'https://www.theiscale.com/myadmin/uploads/courses/Data_science_Course_paid_compressed.png',
    overviewHtml: `
      <p style="color: #64748b; font-size: 16px; line-height: 1.8; margin-bottom: 24px;">
...
,
  ...
  'ai-powered-excel-full-course': {
    ...
    language: 'English',
    category: 'Foundation Courses',
    videoUrl: 'https://www.youtube.com/embed/01sSqCdXhRg',
    cssThumbnailConfig: {
      badge: 'APP + WEB COURSE',
      bgImage: 'https://images.unsplash.com/photo-1551288049-bebda4e38f71?auto=format&fit=crop&w=600&q=80',
      lines: [
...
const TabButton = ({ active, label, onClick }) => (
  <button
    ...
    style={{
      padding: '12px 24px',
      background: active ? 'var(--red)' : '#f8fafc',
...
const Accordion = ({ title, items }) => {
  ...
  return (
    <div style={{ borderBottom: '1px solid #eef0f3', marginBottom: 16 }}>
      <button
      ...
      {isOpen && (
        <div style={{ paddingBottom: 24, display: 'flex', flexDirection: 'column', gap: 16 }}>
          {displayItems.map((item, idx) => (
            <div key={idx} style={{ padding: '16px 24px', background: '#f8fafc', borderRadius: 8, color: '#1e293b', fontWeight: 600, fontSize: 15, display: 'flex', alignItems: 'center', gap: 12 }}>
...
const CourseDetailsPage = ({ courseId, setCurrentPage }) => {
  ...
  const data = coursesDatabase[courseId] || coursesDatabase['data-science-with-generative-ai-course'];

  // Optional: fallback arrays if data doesn't exist for the other courses yet
  const curriculum = data.curriculum || coursesDatabase['data-science-with-generative-ai-course'].curriculum;
  const details = data.details || coursesDatabase['data-science-with-generative-ai-course'].details;
  const highlights = data.highlights || coursesDatabase['data-science-with-generative-ai-course'].highlights;
  ...
  const toolsData = data.tools || coursesDatabase['data-science-with-generative-ai-course'].tools;
  const instructorsData = data.instructors || coursesDatabase['data-science-with-generative-ai-course'].instructors;
  ...
  return (
    <div style={{ background: '#fcfcfd', minHeight: '100vh', fontFamily: 'var(--font-body)', paddingBottom: 100 }}>
      {videoOpen && (
        <div style={{ position: 'fixed', top: 0, left: 0, width: '100%', height: '100%', background: 'rgba(0,0,0,0.9)', zIndex: 10000, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
          <div style={{ width: '80%', maxWidth: 1000, position: 'relative' }}>
            ...
            <div style={{ paddingPadding: '56.25%', position: 'relative', height: 0, overflow: 'hidden' }}>
              <iframe style={{ position: 'absolute', top: 0, left: 0, width: '100%', height: '100%' }} src={videoUrl + "?autoplay=1"} title="Course Video" frameBorder="0" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture" allowFullScreen></iframe>
            </div>
          </div>
        </div>
      )}
      ...
      {/* Hero Section */}
      <section style={{ 
        background: 'linear-gradient(to right, #dcf0ff 0%, #f7d5fe 100%)', 
        padding: '60px 0',
        position: 'relative',
        overflow: 'hidden'
      }}>
        <div className="container" style={{ display: 'flex', gap: 60, alignItems: 'center' }}>
          
          {/* Left Content */}
          <div style={{ flex: 1 }}>
            <div style={{ color: '#64748b', fontSize: 14, fontWeight: 600, marginBottom: 24 }}>
            ...
            <h1 style={{ fontFamily: 'var(--font-display)', fontSize: 48, fontWeight: 900, color: '#0f172a', lineHeight: 1.2, marginBottom: 24 }}>
              {data.title}
            </h1>
            
            <p style={{ color: '#475569', fontSize: 16, lineHeight: 1.6, marginBottom: 32, maxWidth: 600 }}>
              {data.description}
            </p>
            
            <div style={{ display: 'flex', alignItems: 'center', gap: 24, marginBottom: 32 }}>
              <div style={{ background: '#e0e7ff', color: '#1e293b', padding: '6px 16px', borderRadius: 100, fontSize: 14, fontWeight: 700, display: 'flex', alignItems: 'center', gap: 6 }}>
                Popular <span style={{ color: '#64748b', fontWeight: 500 }}>({data.views} No Of Views)</span>
              </div>
            </div>

            <div style={{ display: 'flex', alignItems: 'center', gap: 24, color: '#475569', fontSize: 14, fontWeight: 500 }}>
          ...
          <div style={{ width: 500 }}>
            ...
            >
              ...
              <div style={{ position: 'absolute', top: 0, left: 0, width: '100%', height: '100%', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center' }}>
                <div style={{ position: 'relative', width: 80, height: 80, display: 'flex', alignItems: 'center', justifyContent: 'center', marginTop: -20 }}>
                  ...
                  <div style={{ position: 'relative', width: 80, height: 80, background: '#fff', borderRadius: '50%', display: 'flex', alignItems: 'center', justifyContent: 'center', cursor: 'pointer', boxShadow: '0 10px 30px rgba(0,0,0,0.2)' }}>
            ...
            <button style={{ width: '100%', padding: '16px', background: '#991b1b', color: '#fff', borderRadius: 8, fontWeight: 700, fontSize: 16, border: 'none', cursor: 'pointer', marginBottom: 12, display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 8, transition: 'all 0.2s' }}>
              <Download size={20} /> Download Curriculum <ArrowRight size={20} />
            </button>
            <button style={{ width: '100%', padding: '16px', background: 'var(--red)', color: '#fff', borderRadius: 8, fontWeight: 700, fontSize: 16, border: 'none', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 8, transition: 'all 0.2s', boxShadow: '0 4px 15px rgba(237, 28, 36, 0.3)' }}>
      ...
      {/* Tabs Section */}
      <div style={{ position: 'sticky', top: 70, background: 'rgba(255,255,255,0.95)', backdropFilter: 'blur(10px)', zIndex: 50, borderBottom: '1px solid #e2e8f0', boxShadow: '0 4px 20px rgba(0,0,0,0.02)' }}>
      ...
      <section style={{ padding: '60px 0' }}>
        <div className="container" style={{ display: 'flex', flexDirection: 'column', gap: 60 }}>
          ...
          {/* Overview */}
          <div id="overview" style={{ background: '#fff', padding: 48, borderRadius: 24, boxShadow: '0 10px 40px rgba(0,0,0,0.03)', border: '1px solid #f1f5f9' }}>
            <div style={{ maxHeight: overviewShowMore ? 'none' : 80, overflow: 'hidden', position: 'relative' }}>
              ...
              {!overviewShowMore && <div style={{ position: 'absolute', bottom: 0, left: 0, right: 0, height: 40, background: 'linear-gradient(transparent, white)' }} />}
            </div>
            <button onClick={() => setOverviewShowMore(!overviewShowMore)} style={{ color: 'var(--red)', fontWeight: 800, background: 'none', border: 'none', cursor: 'pointer', fontSize: 16, marginTop: 16 }}>
              {overviewShowMore ? 'Show Less' : 'Show More'}
            </button>
          </div>

          {/* Course Content */}
          <div id="coursecontent" style={{ background: '#fff', padding: 48, borderRadius: 24, boxShadow: '0 10px 40px rgba(0,0,0,0.03)', border: '1px solid #f1f5f9' }}>
            <h2 style={{ fontFamily: 'var(--font-display)', fontSize: 32, fontWeight: 900, color: '#0f172a', marginBottom: 32 }}>Course Content</h2>
            <div style={{ fontWeight: 700, color: '#1e293b', marginBottom: 32, paddingBottom: 24, borderBottom: '1px solid #eef0f3' }}>
              Download Curriculum Details
            </div>
            {curriculum.slice(0, curriculumShowMore ? curriculum.length : 15).map((section, idx) => (
              <Accordion key={idx} title={section.title} items={section.modules} />
            ))}
            {curriculum.length > 15 && (
          ...
          {/* Course Details List */}
          <div id="details" style={{ background: '#fff', padding: 48, borderRadius: 24, boxShadow: '0 10px 40px rgba(0,0,0,0.03)', border: '1px solid #f1f5f9' }}>
            <h2 style={{ fontFamily: 'var(--font-display)', fontSize: 32, fontWeight: 900, color: '#0f172a', marginBottom: 48 }}>Course Details</h2>
              ...
              {details.map((detail, idx) => (
                <div key={idx} style={{ display: 'flex', alignItems: 'center', gap: 40 }}>
                  ...
                  <div style={{ flex: 1, textAlign: 'center' }}>
                    <h3 style={{ fontSize: 24, fontWeight: 800, color: '#1e293b', marginBottom: 16, textDecoration: 'underline', textDecorationColor: '#e2e8f0', textUnderlineOffset: 8 }}>
                      {detail.title}
                    </h3>
                    <p style={{ color: '#64748b', fontSize: 16, lineHeight: 1.6, maxWidth: 600, margin: '0 auto' }}>
                      {detail.desc}
                    </p>
                  </div>
                </div>
          ...
          {/* Highlights */}
          <div id="highlights" style={{ background: '#fff', padding: 48, borderRadius: 24, boxShadow: '0 10px 40px rgba(0,0,0,0.03)', border: '1px solid #f1f5f9' }}>
            <h2 style={{ fontFamily: 'var(--font-display)', fontSize: 24, fontWeight: 900, color: '#0f172a', marginBottom: 48 }}>Highlights</h2>
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(2, 1fr)', gap: '40px 24px' }}>
              {highlights.map((item, idx) => (
                <div key={idx} style={{ display: 'flex', alignItems: 'center', gap: 16 }}>
                  ...
                  <span style={{ color: '#64748b', fontSize: 16, fontWeight: 500 }}>{item.title}</span>
                </div>
              ))}
            </div>
          </div>
          ...
          {/* Certificate */}
          <div id="certificate" style={{ background: '#fff', padding: 48, borderRadius: 24, boxShadow: '0 10px 40px rgba(0,0,0,0.03)', border: '1px solid #f1f5f9' }}>
            <h2 style={{ fontFamily: 'var(--font-display)', fontSize: 24, fontWeight: 900, color: '#0f172a', marginBottom: 48 }}>Certificate</h2>
            <div style={{ display: 'flex', gap: 60, alignItems: 'center' }}>
              <div style={{ flex: 1 }}>
                <p style={{ color: '#64748b', fontSize: 16, lineHeight: 1.8 }}>{certText}</p>
              </div>
              <div style={{ width: 400, textAlign: 'center' }}>
                <img src={certImg} alt="Certificate" style={{ width: '100%', borderRadius: 8, boxShadow: '0 10px 30px rgba(0,0,0,0.1)', cursor: 'zoom-in' }} />
                <p style={{ color: '#64748b', fontSize: 14, marginTop: 12 }}>Click to zoom</p>
              </div>
            </div>
          </div>

          {/* FAQ's */}
          <div id="faq's" style={{ background: '#fff', padding: 48, borderRadius: 24, boxShadow: '0 10px 40px rgba(0,0,0,0.03)', border: '1px solid #f1f5f9' }}>
            <h2 style={{ fontFamily: 'var(--font-display)', fontSize: 24, fontWeight: 900, color: '#0f172a', marginBottom: 32 }}>FAQ's</h2>
            <div>
      ...
      {/* Fees */}
      {feesData && (
        <section id="fees" style={{ padding: '60px 0', background: '#fff', borderTop: '1px solid #f1f5f9' }}>
          <div className="container" style={{ background: '#b91c1c', borderRadius: 24, padding: 48, boxShadow: '0 20px 60px rgba(185,28,28,0.2)' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 40 }}>
              <h2 style={{ fontFamily: 'var(--font-display)', fontSize: 32, fontWeight: 900, color: '#fff' }}>Fees</h2>
              <button style={{ background: '#fff', color: '#b91c1c', border: 'none', padding: '12px 24px', borderRadius: 8, fontWeight: 700, cursor: 'pointer', display: 'flex', alignItems: 'center', gap: 8 }}>
            ...
            <div style={{ background: '#fff', borderRadius: 16, overflow: 'hidden' }}>
              <table style={{ width: '100%', borderCollapse: 'collapse', textAlign: 'center' }}>
                <thead>
                  <tr>
                    <th style={{ padding: 24, textAlign: 'left', borderBottom: '2px solid #e2e8f0', color: '#1e293b', fontSize: 18, width: '40%' }}>
                    ...
                    <th style={{ padding: 24, borderBottom: '2px solid #e2e8f0', borderLeft: '1px solid #e2e8f0' }}>
                      <div style={{ color: '#2dd4bf', fontSize: 18, fontWeight: 800 }}>★ BASIC</div>
                    </th>
                    <th style={{ padding: 24, borderBottom: '2px solid #e2e8f0', borderLeft: '1px solid #e2e8f0' }}>
                      <div style={{ color: '#d97706', fontSize: 18, fontWeight: 800 }}>👑 PREMIUM</div>
                    </th>
                    <th style={{ padding: 24, borderBottom: '2px solid #e2e8f0', borderLeft: '1px solid #e2e8f0', background: '#f5f3ff', position: 'relative' }}>
                      <div style={{ position: 'absolute', top: -16, left: '50%', transform: 'translateX(-50%)', background: '#fff', border: '1px solid #8b5cf6', color: '#8b5cf6', padding: '4px 12px', borderRadius: 100, fontSize: 10, fontWeight: 800, whiteSpace: 'nowrap' }}>★ MOST POPULAR</div>
                ...
                <tbody>
                  {feesData.features.map((feat, idx) => (
                    <tr key={idx}>
                      <td style={{ padding: '16px 24px', textAlign: 'left', borderBottom: '1px solid #e2e8f0', color: '#475569', fontSize: 14, fontWeight: 500 }}>
                        {feat.name}
                      </td>
                      <td style={{ padding: '16px 24px', borderBottom: '1px solid #e2e8f0', borderLeft: '1px solid #e2e8f0' }}>
                      ...
                      <td style={{ padding: '16px 24px', borderBottom: '1px solid #e2e8f0', borderLeft: '1px solid #e2e8f0', background: '#f5f3ff' }}>
                  ...
                  <tr>
                    <td style={{ padding: 24, textAlign: 'left', borderBottom: '1px solid #e2e8f0' }}></td>
                    <td style={{ padding: 24, borderBottom: '1px solid #e2e8f0', borderLeft: '1px solid #e2e8f0' }}>
                      <div style={{ fontSize: 24, fontWeight: 900, color: '#0f172a' }}>{feesData.basic.price}</div>
                      <button style={{ background: '#fff', border: '1px solid #2dd4bf', color: '#2dd4bf', padding: '8px 24px', borderRadius: 8, fontWeight: 700, marginTop: 12, cursor: 'pointer' }}>Get Started</button>
                    </td>
                    <td style={{ padding: 24, borderBottom: '1px solid #e2e8f0', borderLeft: '1px solid #e2e8f0' }}>
                      <div style={{ fontSize: 24, fontWeight: 900, color: '#0f172a' }}>{feesData.premium.price}</div>
                    ...
                    <td style={{ padding: 24, borderBottom: '1px solid #e2e8f0', borderLeft: '1px solid #e2e8f0', background: '#f5f3ff' }}>
                      <div style={{ fontSize: 24, fontWeight: 900, color: '#0f172a' }}>{feesData.pro.price}</div>
            ...
            <div style={{ marginTop: 40, color: '#fff' }}>
              ...
              <div style={{ marginTop: 16 }}>
                <button style={{ background: '#fff', color: '#b91c1c', border: 'none', padding: '12px 32px', borderRadius: 8, fontWeight: 700, cursor: 'pointer', display: 'flex', alignItems: 'center', gap: 8, float: 'right', marginTop: -40 }}>
      ...
      {/* Tools */}
      {toolsData && (
        <section id="tools" style={{ padding: '60px 0', background: '#f8fafc' }}>
          <div className="container" style={{ background: '#fff', borderRadius: 24, padding: 48, boxShadow: '0 10px 40px rgba(0,0,0,0.03)' }}>
            <h2 style={{ fontFamily: 'var(--font-display)', fontSize: 24, fontWeight: 900, color: '#0f172a', marginBottom: 40 }}>Tools</h2>
      ...
      {instructorsData && (
        <section id="instructor" style={{ padding: '60px 0' }}>
          <div className="container" style={{ background: '#fff', borderRadius: 24, padding: 48, boxShadow: '0 10px 40px rgba(0,0,0,0.03)' }}>
            <h2 style={{ fontFamily: 'var(--font-display)', fontSize: 24, fontWeight: 900, color: '#0f172a', marginBottom: 40 }}>Instructor</h2>
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: 24 }}>
              {instructorsData.map((inst, idx) => (
                <div key={idx} style={{ border: '1px solid #e2e8f0', borderRadius: 16, padding: 32, textAlign: 'center', boxShadow: '0 4px 15px rgba(0,0,0,0.02)', backgroundColor: '#fff' }}>
                  ...
                  <h3 style={{ fontSize: 20, fontWeight: 800, color: '#1e293b', marginBottom: 4 }}>{inst.name}</h3>
                  <p style={{ color: 'var(--red)', fontSize: 14, fontWeight: 600, marginBottom: 16 }}>{inst.role}</p>
                  <div style={{ display: 'inline-flex', alignItems: 'center', justifyContent: 'center', width: 40, height: 40, backgroundColor: '#f1f5f9', borderRadius: '50%', color: '#0077b5' }}>
      ...
      {/* Review */}
      {reviewsData && (
        <section id="review" style={{ padding: '60px 0', background: '#f8fafc' }}>
          <div className="container" style={{ background: '#fff', borderRadius: 24, padding: 48, boxShadow: '0 10px 40px rgba(0,0,0,0.03)' }}>
            <h2 style={{ fontFamily: 'var(--font-display)', fontSize: 24, fontWeight: 900, color: '#0f172a', marginBottom: 40 }}>Review</h2>
            <div style={{ display: 'flex', gap: 24, overflowX: 'auto', paddingBottom: 24, scrollbarWidth: 'none' }}>
              {reviewsData.map((rev, idx) => (
                <div key={idx} style={{ flexShrink: 0, width: 260, padding: 24, backgroundColor: 'white', borderRadius: 24, border: '1px solid #e2e8f0', textAlign: 'center', boxShadow: '0 4px 20px rgba(0,0,0,0.05)' }}>
                  ...
                  <h4 style={{ fontSize: 18, fontWeight: 700, color: '#1e293b', marginBottom: 4 }}>{rev.name}</h4>
                  <p style={{ fontSize: 14, color: '#64748b', marginBottom: 20 }}>Placed at</p>
      ...
      <section style={{ padding: '60px 0' }}>
        <div className="container">
          ...
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 40 }}>
            <h2 style={{ fontFamily: 'var(--font-display)', fontSize: 32, fontWeight: 900, color: '#0f172a' }}>More Course By <span style={{ color: 'var(--red)' }}>Data Science Courses</span></h2>
            <button style={{ border: '1px solid #e2e8f0', background: '#fff', padding: '10px 24px', borderRadius: 100, fontWeight: 600, color: '#475569', cursor: 'pointer' }}>View All Course</button>
          </div>
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(320px, 1fr))', gap: 32 }}>
            {[1, 2, 3].map((_, i) => (
              <div key={i} style={{ background: '#fff', borderRadius: 16, overflow: 'hidden', boxShadow: '0 4px 20px rgba(0,0,0,0.05)', border: '1px solid #f1f5f9' }}>
                ...
                <div style={{ padding: 24 }}>
                  <h3 style={{ fontSize: 18, fontWeight: 800, color: '#1e293b', marginBottom: 12 }}>AI Engineer Advance Program</h3>
                  <div style={{ display: 'flex', gap: 16, color: '#64748b', fontSize: 12, marginBottom: 24 }}>
                  ...
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                      <span style={{ fontWeight: 800, fontSize: 20, color: '#0f172a' }}>₹33,999</span>
                      <span style={{ textDecoration: 'line-through', color: '#94a3b8', fontSize: 14 }}>₹34,999</span>
                    </div>
                    <button style={{ color: '#0f172a', fontWeight: 700, background: 'none', border: 'none', cursor: 'pointer', display: 'flex', alignItems: 'center', gap: 4 }}>
