...
const AppContent = () => {
  ...
  const dashboardPages = ['dashboard', 'my-profile', 'enrolled-courses', 'enrolled-events', 'settings', 'test-series-result'];
  const isDashboard = dashboardPages.includes(currentPage);
  ...
  return (
    <div style={{ minHeight: '100vh', display: 'flex', flexDirection: 'column' }}>
      {showNavAndFooter && <Navbar currentPage={currentPage} setCurrentPage={handleNavigate} />}
      <main style={{ flex: 1 }}>
        {isDashboard ? (
          <DashboardLayout activeTab={currentPage} setCurrentPage={handleNavigate}>
            <Routes>
